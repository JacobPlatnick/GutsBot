...
def helper(p1, p2, p3, p4):
    ...
    temp = (p4 - p1) * p2 * (1 - p4) * (1 - p4) * np.array([-3, 1, 1, 1])
    value = value + temp

    #case 25: h h d h fair play
    temp = (1 - p4) * (1 - p4) * p3 * (1 - p4) * np.array([-1 / 3, -1/3, 1, -1 / 3])
    value = value + temp
...
N = 1001
M = 1001
A = [[alpha(i/(N - 1), i/(N - 1), j/(M - 1), j/(M - 1))[0] +alpha(i/(N - 1), i/(N - 1), j/(M - 1), j/(M - 1))[1]
